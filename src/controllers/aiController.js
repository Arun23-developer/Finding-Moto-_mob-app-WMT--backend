...
const roleGuidance = {
    buyer: 'You are Finding Moto AI assistant for buyers. Help with bike problems, price estimates, spare parts, and buying tips only.',
    seller: 'You are Finding Moto AI assistant for sellers. Help with listing quality, selling price, margins, spare parts pricing, and marketplace growth only.',
    mechanic: 'You are Finding Moto AI assistant for mechanics. Help with legal repair, diagnostics, service pricing, tools, and spare parts only.',
};
const roleKeywords = {
    buyer: [
...
const abusiveOrUnwantedPatterns = [
    /\bi\s*love\s*you\b/i,
    /\blove\s+you\b/i,
    /\bromance\b/i,
    /\bromantic\b/i,
...
const containsAnyKeyword = (message, keywords) => {
...
const buildRegexFromTerms = (terms) => {
...
const isSellerReportRequest = (message) => {
    const lower = message.toLowerCase();
    return REPORT_KEYWORDS.some((keyword) => lower.includes(keyword));
};
const isMechanicReportRequest = (message) => {
    const lower = message.toLowerCase();
    return REPORT_KEYWORDS.some((keyword) => lower.includes(keyword));
};
const formatLkr = (value) => `LKR ${Math.round(value).toLocaleString()}`;
const getSellerCurrentReport = async (sellerId) => {
    const now = new Date();
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    const [products, totalOrders, pendingOrders, completedOrders, revenueAgg, monthlyRevenueAgg, ordersForAverage,] = await Promise.all([
        Product_1.default.find({ seller: sellerId, type: 'product' })
            .select('name price stock status sales')
            .lean(),
        Order_1.default.countDocuments({ seller: sellerId }),
        Order_1.default.countDocuments({ seller: sellerId, status: 'pending' }),
        Order_1.default.countDocuments({ seller: sellerId, status: { $in: ['delivered', 'confirmed', 'shipped'] } }),
        ...
        Order_1.default.find({ seller: sellerId, status: { $in: ['delivered', 'confirmed', 'shipped'] } })
            .select('totalAmount')
            .lean(),
    ]);
    const productIds = products.map((p) => p._id);
    const reviews = productIds.length > 0
        ? await Review_1.default.find({ productId: { $in: productIds } }).select('rating').lean()
        : [];
...
const buildSellerCurrentReportText = (report) => {
...
const getMechanicCurrentReport = async (mechanicId) => {
    const now = new Date();
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    const [totalServices, activeServices, totalJobs, pendingJobs, confirmedJobs, shippedJobs, completedJobs, revenueAgg, monthlyRevenueAgg, jobValues, topRequestedServices, mechanicProducts,] = await Promise.all([
        ...
        Order_1.default.countDocuments({ seller: mechanicId, status: 'pending' }),
        Order_1.default.countDocuments({ seller: mechanicId, status: 'confirmed' }),
        Order_1.default.countDocuments({ seller: mechanicId, status: 'shipped' }),
        Order_1.default.countDocuments({ seller: mechanicId, status: 'delivered' }),
        Order_1.default.aggregate([
        ...
        Order_1.default.find({ seller: mechanicId, status: { $in: ['delivered', 'confirmed', 'shipped'] } })
            .select('totalAmount')
            .lean(),
        Order_1.default.aggregate([
        ...
        Product_1.default.find({ seller: mechanicId }).select('_id').lean(),
    ]);
...
const buildMechanicCurrentReportText = (report) => {
...
const getSellerAnalyticsSnapshot = async (sellerId, message, productHint) => {
    ...
    const [totalMarketplaceProducts, totalMarketplaceAvailableProducts, matchingDocs, sellerMatchingDocs, sellerUnitsSoldAggregate] = await Promise.all([
        Product_1.default.countDocuments({ type: 'product' }),
        Product_1.default.countDocuments({ type: 'product', stock: { $gt: 0 }, status: 'active' }),
    ...
    const prices = matchingDocs.map((p) => p.price).filter((price) => Number.isFinite(price));
    const avg = prices.length > 0 ? safeRound(prices.reduce((sum, price) => sum + price, 0) / prices.length) : 0;
    const min = prices.length > 0 ? Math.min(...prices) : 0;
    const max = prices.length > 0 ? Math.max(...prices) : 0;
    const median = computeMedian(prices);
    const sellerPrices = sellerMatchingDocs.map((p) => p.price).filter((price) => Number.isFinite(price));
...
const isGlobalBlockedMessage = (message) => {
    return abusiveOrUnwantedPatterns.some((pattern) => pattern.test(message));
};
const getRoleRedirectMessage = (role) => {
    if (role === 'buyer') {
...
const extractSellerDescriptionTarget = (message) => {
...
const buildFriendlyFallback = (role, message) => {
    const lower = message.toLowerCase();
...
const getScopeRedirectInsteadOfBlock = (role) => {
    if (role === 'buyer') {
...
const buildPrompt = (role, message, sellerSnapshot, hasImage) => {
    ...
    const sellerBlock = role === 'seller' && sellerSnapshot
        ? [
            ...
            ...sellerSnapshot.topMatchingProducts.map((item, index) => `  ${index + 1}. ${item.name} | ${item.brand} | ${item.category} | LKR ${item.price.toLocaleString()} | Stock ${item.stock}`),
        ].join('\n')
        : '';
    return [
        guidance,
...
const askAI = async (req, res) => {
    try {
        ...
        const { message, role, imageDataUrl, productName } = req.body;
